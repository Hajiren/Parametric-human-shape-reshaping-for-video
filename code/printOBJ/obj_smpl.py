...
pkl_paths=sorted(glob(join("./pkl/","modify_image*_smpl.pkl")))
for ind,pkl_path in enumerate(pkl_paths):
	m = load_model(pkl_path)
	ind=int(re.findall(r"./pkl/modify_image(.+?)_smpl.pkl",pkl_path)[0])
	## Assign random pose and shape parameters

	## Write to an .obj file
	outmesh_path = './image%d_object.obj'%ind
...
'''
pkl_paths=sorted(glob(join("pkl/","*[0-9].pkl")))
for ind,pkl_path in enumerate(pkl_paths):
	m = load_model(pkl_path)
	ind=int(re.findall(r"pkl/result_test(.+?).pkl",pkl_path)[0])
	## Assign random pose and shape parameters

	## Write to an .obj file
	outmesh_path = './smpl%d.obj'%ind
	with open( outmesh_path, 'w') as fp:
	    for v in m.r:
		fp.write( 'v %f %f %f\n' % ( v[0], v[1], v[2]) )
